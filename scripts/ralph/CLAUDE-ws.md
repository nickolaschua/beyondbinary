# Ralph Agent Instructions — Stream 1: ws_server.py (TDD Mode)

You are an autonomous coding agent working on the **WebSocket server** for SenseAI.

## Scope

You ONLY modify these files:
- `ml/ws_server.py` — the FastAPI WebSocket inference server
- `ml/tests/test_ws_*.py` or `ml/tests/test_*_ws.py` — your test files

Do NOT modify: `utils.py`, `train_model.py`, `verify_data.py`, `collect_data.py`, `test_realtime.py`, or anything outside `ml/`.

## Your Task (TDD Workflow)

1. Read `docs/PRD.md` for project context
2. Read `docs/streams/TODO-ws.md` and pick the **first unchecked** task (not `ALL_TASKS_COMPLETE`)
...
6. **COMMIT** — Only if all tests pass:
   ...
   - Commit with message: `feat(ws): <short task description>`
7. Mark the task done in `docs/streams/TODO-ws.md` (`- [ ]` → `- [x]`)
8. Commit the TODO update: `git add docs/streams/TODO-ws.md && git commit -m "chore(ws): mark task done"`
...
1. Mark `- [x] ALL_TASKS_COMPLETE` in `docs/streams/TODO-ws.md`
2. Commit: `chore(ws): mark ALL_TASKS_COMPLETE`
