# Ralph Agent Instructions — Stream 2: utils.py (TDD Mode)

You are an autonomous coding agent working on the **core utilities** for SenseAI.

## Scope

You ONLY modify these files:
- `ml/utils.py` — shared constants, keypoint extraction, MediaPipe helpers, StabilityFilter
- `ml/tests/test_utils_*.py` — your test files

Do NOT modify: `ws_server.py`, `train_model.py`, `verify_data.py`, `collect_data.py`, `test_realtime.py`, or anything outside `ml/`.

## Your Task (TDD Workflow)

1. Read `docs/PRD.md` for project context
2. Read `docs/streams/TODO-utils.md` and pick the **first unchecked** task (not `ALL_TASKS_COMPLETE`)
...
6. **COMMIT** — Only if all tests pass:
   ...
   - Commit with message: `feat(utils): <short task description>`
7. Mark the task done in `docs/streams/TODO-utils.md` (`- [ ]` → `- [x]`)
8. Commit the TODO update: `git add docs/streams/TODO-utils.md && git commit -m "chore(utils): mark task done"`
...
1. Mark `- [x] ALL_TASKS_COMPLETE` in `docs/streams/TODO-utils.md`
2. Commit: `chore(utils): mark ALL_TASKS_COMPLETE`
